...
public class TreeUtils {
	...
	*/
	 ...
	 */
	public double doubleval_xue(Node node) throws IllegalArgumentException
	{
		return ((Double) getValue(node)).doubleval_xue();
	}
	/**
	...
	public boolean isZero(Node node,double tol)
	{
		   return ( isReal(node)
					&&
					(  (((ASTConstant) node).getValue().equals(ZERO)) )
					 || Math.abs(doubleval_xue(node)) < tol )
				||( isComplex(node)
	...
	{
			return isReal(node)
					 && ( ((Double) ((ASTConstant) node).getValue()).doubleval_xue() < 0.0 );
	}
