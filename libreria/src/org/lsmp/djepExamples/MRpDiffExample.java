...
/**
 * An example of using MRpeval_x with differentation.
 * 
 * @author Rich Morris
 * Created on 01-Apr-2005
 */
public class MRpDiffExample {
	...
	public void compile(String text)
	{
		...
		try
		{
			...
			// successfully eval_xuate topEqn, dx, and dy
			...
			// Compile all equations needed for successful eval_xuation
			// of top, dx, dy
		...
		for(int i=0;i<=xsteps;++i) {
			...
			for(int j=0;j<=ysteps;++j) {
				...
				for(int k=0;k<allCommands.length-3;++k)
					res = mrpe.eval_xuate(allCommands[k]);
				res = mrpe.eval_xuate(allCommands[allCommands.length-3]);
				topRes = (double []) res.toArray();
				res = mrpe.eval_xuate(allCommands[allCommands.length-2]);
				dxRes = (double []) res.toArray();
				res = mrpe.eval_xuate(allCommands[allCommands.length-1]);
