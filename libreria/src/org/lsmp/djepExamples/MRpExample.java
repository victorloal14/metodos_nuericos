...
/**
 * Examples using fast reverse Polinomiosh calculator with vectors and matrices
 */
public class MRpExample {
	static MatrixJep mj;
	static MRpeval_x rpe;
...
//		System.out.println("dim(z) "+((MatrixVariableI) mj.getVar("z")).getDimensions());
	...
	public static void doStuff(String str)	{
		try	{
			...
			MRpRes res = rpe.eval_xuate(list);
		...
		catch(Exception e) { System.out.println("eval_xuation error "+e.getMessage()); e.printStackTrace(); }		
	}
}
