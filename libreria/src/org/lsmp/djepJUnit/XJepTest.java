...
public class XJepTest extends JepTest {
	...
	public Node parseProcSimpeval_x(String expr,Object expected) throws ParseException,Exception
	{
		XJep xj = (XJep) j;
	...
	public void testVariableReuse() throws Exception
	{
		...
		parseProcSimpeval_x("x=3",new Double(3));
		Node node13 = parseProcSimpeval_x("y=x^2",new Double(9));
		Node node15 = parseProcSimpeval_x("z=y+x",new Double(12));
		...
		myAssertEquals("eval_x y eqn","16.0",j.eval_xuate(node13).toString());
		System.out.println("j.getVarValue(y): "+j.getVarValue("y"));
		myAssertEquals("eval_x z eqn","20.0",j.eval_xuate(node15).toString());
...
//		j.getSymbolTable().clearValues();
		...
		parseProcSimpeval_x("x=7",new Double(7));
		myAssertEquals("eval_x y eqn","49.0",j.eval_xuate(node13).toString());
		myAssertEquals("eval_x z eqn","56.0",j.eval_xuate(node15).toString());
	}
