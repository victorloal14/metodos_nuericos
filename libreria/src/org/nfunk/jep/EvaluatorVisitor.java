 ...
 */
public class eval_xuatorVisitor implements ParserVisitor, eval_xuatorI {
	/** Stack used for eval_xuating the expression */
	...
	/** Flag for errors during eval_xuation */
	 ...
	 * <p>
	 * An exception is thrown, if an error occurs during eval_xuation.
	 * @return The value of the expression as an object.
	 * @throws ParseException if there is a problem with the eval_xuation.
	 */
	public Object getValue(Node topNode,SymbolTable symTab_in)
		...
		// eval_xuate by letting the top node accept the visitor
	...
	*/
	...
	/**
	 * eval_xuates a given node, in the current context.
	 * @param node The node to eval_xuate
	 * @return result of the eval_xuation
	 */
	public Object eval_x(Node node) throws ParseException {
		node.jjtAccept(this,null);
		return stack.pop();
	}
	
	/**
	 * eval_xuates a PostfixMathCommandI with given arguments.
	 * Not used in normal use.
	 * 
	 * @param pfmc the command to eval_xuate.
	 * @param children the parameters to the function.
	 * @return the value of the function
	 * @throws ParseException
	 */
	public Object eval_x(PostfixMathCommandI pfmc,Node children[]) throws ParseException 
	{
		if (pfmc instanceof Specialeval_xuationI) {
			...
			node.jjtClose();
			return ((Specialeval_xuationI) pfmc).eval_xuate(
				node,null,this,new Stack(),this.symTab);
		}
		if(pfmc instanceof Callbackeval_xuationI) {
		    ASTFunNode node = new ASTFunNode(ParserTreeConstants.JJTFUNNODE);
			node.setFunction("TmpFun",pfmc);
			node.jjtOpen();
			for(int i=0;i<children.length;++i) 
			    node.jjtAddChild(children[i],i);
			node.jjtClose();
			Object val = ((Callbackeval_xuationI) pfmc).eval_xuate(node,this);
			return val;
		}
	...
	/**
	 * This method should never be called when eval_xuation a normal
	 * expression.
	 */
	...
	/**
	 * This method should never be called when eval_xuating a normal
	 * expression.
	 */
	public Object visit(ASTStart node, Object data) throws ParseException {
		throw new ParseException("Start node encountered during eval_xuation");
	}

	/**
	 * Visit a function node. The values of the child nodes
	 * are first pushed onto the stack. Then the function class associated
	 * with the node is used to eval_xuate the function.
	 * <p>
	 * If a function implements Specialeval_xuationI then the
	 * eval_xuate method of PFMC is called.
	 */
	public Object visit(ASTFunNode node, Object data) throws ParseException {
		...
		// Some operators (=) need a special method for eval_xuation
		// as the pfmc.run method does not have enough information
		// in such cases we call the eval_xuate method which passes
		// all available info. Note eval_xuating the children is
		// the responsibility of the eval_xuate method. 
		if (pfmc instanceof Specialeval_xuationI) {
			return ((Specialeval_xuationI) pfmc).eval_xuate(
		...
		// eval_xuate all children (each leaves their result on the stack)
	...
	/**
	 * Tests whether null variable values are trapped by eval_xuator. 
