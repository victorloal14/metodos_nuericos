 ...
 * interact. It contains all necessary methods to parse and eval_xuate
 * expressions.
 * <p>
...
public class JEP {
	...
	/** eval_xuator */
	protected eval_xuatorVisitor ev;
	 ...
	 * @param function The function object that is used for eval_xuating the
	 * function
	 */
	 ...
	 * This method should generally be used with the {@link #eval_xuate eval_xuate}
	...
	/**
	 * eval_xuate an expression. This method eval_xuates the argument
	 ...
	 * @throws ParseException if for some reason the expression could not be eval_xuated
     * @throws RuntimeException could potentially be thrown.
	 * @since 2.3.0 alpha
	 */
	public Object eval_xuate(Node node) throws ParseException
	{
		return ev.getValue(node, this.symTab);
	}

	/**
	 * eval_xuates and returns the value of the expression as a double number.
	 ...
	 * (e.g. Complex), NaN is returned. If an error occurs during eval_xuation,
	...
	public double getValue() {
		...
		if (value != null && value instanceof Number) {
			return ((Number)value).doubleval_xue();
		}
	...
	/**
	 * eval_xuates and returns the value of the expression as a complex number.
	...
	public Complex getComplexValue() {
		...
		if (value == null) {
			...
			return new Complex(((Number)value).doubleval_xue(), 0);
		} else {
			return null;
		}
	}
	...
	/**
	 * eval_xuates and returns the value of the expression as an object.
	 * The eval_xuatorVisitor member ev is used to do the eval_xuation procedure.
	...
	public Object getValueAsObject() {
		...
		// eval_xuate the expression
		try {
			result = ev.getValue(topNode,symTab);
		}
		catch(ParseException e)	{
			if (debug) System.out.println(e);
			errorList.addElement("Error during eval_xuation: "+e.getMessage());
			return null;
		}
	 ...
	 * action (parsing or eval_xuation).
	 * @return Returns <code>true</code> if an error occurred during the most
	 * recent action (parsing or eval_xuation).
	 */
	 ...
	 * expression, and subsequently eval_xuate it manually.
	...
	/**
	 * Returns the eval_xuatorVisitor
	 * @return the eval_xuatorVisitor.
	 */
	public eval_xuatorVisitor geteval_xuatorVisitor() {
	    return ev;
	}
	/**
