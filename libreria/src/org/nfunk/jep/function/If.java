 ...
 * <p>
 * This function performs lazy eval_xuation so that
 * only posExpr or negExpr will be eval_xuated.
 ...
 * <p>
 * This function implements the Specialeval_xuationI interface
 ...
 */
public class If extends PostfixMathCommand implements Callbackeval_xuationI {
    ...
    */
	...
	public Object eval_xuate(Node node,eval_xuatorI pv) throws ParseException
	{
		...
		Object condVal = pv.eval_x(node.jjtGetChild(0));
		...
		if(condVal instanceof Boolean)
		{
			if(((Boolean) condVal).booleanValue())
				return pv.eval_x(node.jjtGetChild(1));
			return pv.eval_x(node.jjtGetChild(2));
		}
		else if(condVal instanceof Complex)
			val = ((Complex) condVal).re();
		else if(condVal instanceof Number)
			val = ((Number) condVal).doubleval_xue();
		else
		...
		if(val>0.0)
			...
			return pv.eval_x(node.jjtGetChild(2));
		return pv.eval_x(node.jjtGetChild(3));
	}
}
