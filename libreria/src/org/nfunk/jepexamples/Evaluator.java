...
/**
 * eval_xuator - JEP Example Applet
 * Copyright (c) 2004 Nathan Funk
 *
 * @author Nathan Funk

HTML code for running the applet:
<applet code="org/nfunk/jepexamples/eval_xuator.class" width=400 height=200>
</applet>
...
/**
 * This applet is an simple example for how JEP can be used to eval_xuate
 * expressions. It also displays the different options, and the effects of
 * their settings.
 */
public class eval_xuator extends Applet {
	...
	public static void main(String args[]) {
		eval_xuator a = new eval_xuator();
		a.init();
		a.start();

		Frame f = new Frame("eval_xuator");
		f.add("Center", a);
		f.setSize(400,200);
		f.addWindowListener(
			new WindowAdapter() {
...
//		f.show(); 
	...
	private void xFieldTextValueChanged() {
		...
		try {
			xValue = Double.valueOf(xField.getText()).doubleval_xue();
