...
public class BugsTest extends TestCase {
	 ...
	 * eval_xuation. This is expected behaviour since the run() method is
	...
	public void testHasError() {		
		...
		// test type error (should have error after eval_xuation)
	...
	private class CustFunc extends PostfixMathCommand
	{
		...
		public void run(Stack inStack) throws ParseException 
		{
			...
			if (param instanceof Number && ((Number)param).doubleval_xue() > 0) {
				inStack.push(param);
			} else {
	 ...
	 * 
	 * JEP 2.4.0 checks the <code>errorList</code> variable before eval_xuating 
	 ...
	 * getValueAsObject. If the first eval_xuation fails (after a successful parse)
	...
	public void testBug1563324() {
		...
		// add a variable with a value that causes eval_xuation to fail
		// (the Random type is not supported by the abs function)
		jep.addVariable("x", new java.util.Random()); 
		Object result = jep.getValueAsObject();
		// eval_xuation should have failed
		Assert.assertTrue(jep.hasError());
		
		// change the variable value to a value that should be eval_xuated
		jep.addVariable("x", -1);
		// ensure that it is eval_xuated correctly
		result = jep.getValueAsObject();
		Assert.assertTrue((result instanceof Double) && ((Double)result).doubleval_xue() == 1.0);
	}
}
