...
/**
 * This class is designed for testing the validity of JEP eval_xuations.
 * Expressions from a text file are eval_xuated with JEP in pairs of two, and
 ...
 * <pre>1+2
 *3.</pre>
 * The expressions '1+2' and '3' are eval_xuated with JEP and the results compared.
 */
public class JEPTest extends TestCase {
	...
	/**
	 * Loads the file specified in fileName. eval_xuates the expressions listed
	...
	public void testWithFile(String fileName) {
		...
		println("eval_xuating and comparing expressions...");
		while (true) {
	 ...
	 * eval_xuation of that line.
	 * @return eval_xuated line. Returns null when the end of the file
	 *         is reached.
	...
	private Object parseNextLine(BufferedReader reader) throws Exception {
		...
		// did an error occur while eval_xuating?
		if (value == null || myParser.hasError()) {
			errorStr = myParser.getErrorInfo();
			throw new Exception("Error while eval_xuating line " + lineCount + ": " + errorStr);
		}
	...
	private boolean equal(Object param1, Object param2)
	{
		...
		if ((param1 instanceof Number) && (param2 instanceof Number)) {
			return Math.abs(((Number)param1).doubleval_xue()-((Number)param2).doubleval_xue())
					< tolerance;
		}
